    ...
    <div class="stats">
      <div class="player-stats">
        ...
        <div id="health-bar" class="health-bar">
          ...
          <p>
            <img src="@/assets/images/adventura/heart-symbol.png" class="health-icon" />
            {{ health }}
          </p>
        </div>
        <div id="mana-bar" class="mana-bar">
          ...
          <p>
            <img src="@/assets/images/adventura/mana-symbol.png" class="mana-icon" />
            {{ mana }}
          </p>
        </div>
        <div class="coins">
          <p ref="coinText">
            {{ visibleCoinAmount }}<img src="@/assets/images/adventura/coin.png" ref="coin" id="coin" />
          </p>
        </div>
      </div>
      ...
      <div class="enemy-stats">
        ...
        <div class="weaknesses-and-strengths">
          <div class="weaknesses">
            ...
            <p v-for="(weakness, i) in enemyWeakness" v-bind:key="'A' + i" class="weakness">
              {{ weakness }}
            </p>
          </div>
          <div class="strengths">
            <p id="strength">Strengths:</p>
            <p v-for="(strength, i) in enemyStrength" v-bind:key="'B' + i" class="strength">
              {{ strength }}
            </p>
          </div>
        </div>
...
export default defineComponent({
  ...
  props: {
    ...
    maxEnemyHealth: { type: Number, required: true },
    mana: { type: Number, required: true },
    enemyMana: { type: Number, required: true },
    enemyWeakness: { type: Array, required: true },
    enemyStrength: { type: Array, required: true },
    getNewEnemy: { type: Boolean, required: true },
    coins: { type: Number, required: true },
    visibleCoinAmount: { type: String, required: true },
    currentArea: { type: Object, required: true },
    inBattle: { type: Boolean, required: true }
  },
  ...
  watch: {
    ...
    health: function() {
      ...
      health.style.width = Math.round((this.health / this.maxHealth) * 100).toString() + '%';
    },
    mana: function() {
      const mana = this.$refs.mana as HTMLElement;
      mana.style.width = Math.round((this.mana / this.maxMana) * 100).toString() + '%';
    },
    enemyHealth: function() {
      const enemyHealth = this.$refs.enemyHealth as HTMLElement;
      setTimeout(() => {
        enemyHealth.style.width = Math.round((this.enemyHealth / this.maxEnemyHealth) * 100).toString() + '%';
      }, 3);
    },
    enemyMana: function() {
      const enemyMana = this.$refs.enemyMana as HTMLElement;
      enemyMana.style.width = Math.round((this.enemyMana / this.maxEnemyMana) * 100).toString() + '%';
    },

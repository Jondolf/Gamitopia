  ...
  <div id="adventura">
    ...
    <div class="gameContainer" ref="gameContainer">
      ...
      <BattleArea
        ...
        @newEnemyGotten="getNewEnemy = false"
        @enemy="setEnemyName"
        @getMana="getMana"
      ...
      <StartMenu
        ...
        @toggleSfx="sfxOn = !sfxOn"
      />
      <AreaMap
        v-if="currentView === 'Map'"
        :currentEnemy="currentEnemy"
        :bossesDefeated="bossesDefeated"
        @newCurrentArea="setCurrentArea"
        @openShop="currentView = 'Shop'"
        @openStartMenu="currentView = 'Start menu'"
        @bossArea="setBossArea"
      />
      <Shop
        ...
        :maxMana="maxMana"
        @itemBought="buyItem"
        @healthUpgraded="maxHealth += 1"
        @manaUpgraded="maxMana += 1"
        @swordUpgraded="upgradeItem(ironSword, 'ironSword')"
        @fireUpgraded="upgradeItem(fireScroll, 'fire')"
        @waterUpgraded="upgradeItem(waterScroll, 'water')"
        @iceUpgraded="upgradeItem(iceScroll, 'ice')"
        @earthUpgraded="upgradeItem(earthScroll, 'earth')"
        @airUpgraded="upgradeItem(airScroll, 'air')"
        @lightningUpgraded="upgradeItem(lightningScroll, 'lightning')"
        @healUpgraded="upgradeItem(healScroll, 'heal')"
...
export default Vue.extend({
  ...
  methods: {
    ...
    checkWeaknessesAndStrengths(
      amount: number,
      manaToConsume: number,
      ...
      if (amount > 0) {
        ...
        this.damageInflicted = Math.ceil(
          amount * (enemyWeakness - enemyStrength)
        ).toString();
      }
      if (amount === 0) {
        this.enemyHealth -= 0;
  ...
  watch: {
    ...
    visibleCoinAmount: function() {
      if (
        +this.visibleCoinAmount >= 1000000 &&
      ...
      if (
        +this.visibleCoinAmount >= 1000000000 &&
        +this.visibleCoinAmount < 1000000000000
      ) {
        this.visibleCoinAmount =
